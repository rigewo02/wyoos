...
FileAllocationTable32::FileAllocationTable32(AdvancedTechnologyAttachment *hd, uint32_t partitionOffset) {
    ...
    this->fatSize = bpb.tableSize;
    this->dataStart = fatStart + fatSize*bpb.fatCopies;

    this->rootStart = dataStart + bpb.sectorsPerCluster*(bpb.rootCluster - 2);
    this->sectorsPerCluster = bpb.sectorsPerCluster;
    Read();
}

void FileAllocationTable32::Read() {
